...
<%@ attribute name="callback" type="java.lang.Boolean" required="false" description=""%>
...
<script type="text/javascript">
	$("#${id}Button, #${id}Name").click(function(){
		...
		jp.openUserSelectDialogById(${isMultiSelected? true:false},param,function(ids,names,phones,mobiles){
			$("#${id}Id").val(ids.replace(/u_/ig,""));
			$("#${id}Name").val(names);
			$("#${id}Name").focus();
			if("${callback}"){
				callback(phones,mobiles);
			}
		})
	});
