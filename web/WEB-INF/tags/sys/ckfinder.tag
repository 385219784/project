...
<script type="text/javascript">
	...
	function ${input}Preview(){
		...
		for (var i=0; i<urls.length; i++){
			if (urls[i]!=""){//<c:if test="${type eq 'thumb' || type eq 'images'}">
				...
				li = "<li><a href=\""+urls[i]+"\" id='files"+ i +"' url=\""+urls[i]+"\" target=\"_blank\">"+decodeURIComponent(urls[i].substring(urls[i].lastIndexOf("/")+1))+"</a>";//</c:if>
	...
	${input}Preview();
	/* &nbsp;&nbsp;&nbsp;<a class='btn btn-xs btn-info' onclick='downloadFiles("+ i +")'>下载</a> */
	//文件下载
	/* function downloadFiles(idx){
		var href = $("#files"+idx).attr("url");
		var fileName = $("#files"+idx).text();
		jp.post("${ctx}/paper/identifyingBill/downloadFiles",{href:href,fileName:fileName},
		function(data){
			if(data != ""){
				jp.error(data);
			}
