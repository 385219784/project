...
<%@ attribute name="clear" type="java.lang.String" required="false" description="清除项,使用,分割"%>
<%@ attribute name="powerGridClear" type="java.lang.String" required="false" description="清除项,使用,分割"%>
<%@ attribute name="powerGridInfo" type="java.lang.Boolean" required="false" description="回调函数"%>
<%@ attribute name="areaCallBack" type="java.lang.Boolean" required="false" description="回调函数"%>
<%@ attribute name="getHistoryprice" type="java.lang.Boolean" required="false" description="回调函数"%>
...
$(document).ready(function(){
	$("#${id}Button, #${id}Name").click(function(){
		...
		top.layer.open({
		    ...
		    content: "${ctx}/tag/treeselect?url="+encodeURIComponent("${url}")+param+"&module=${module}&checked=${checked}&extId=${extId}&isAll=${isAll}&allowSearch=${allowSearch}" ,
		    btn: ['确定', '关闭']
    	       ,yes: function(index, layero){ //或者使用btn1
						...
						if ("${clear}" != "") {
							var cl = "${clear}";
							var oldVal = $("#${id}Id").val();
							var newVal = ids.join(",").replace(/u_/ig,"");
							if (oldVal != newVal) {
								var clears = [];
								clears = cl.split(",");
						...
						if("${powerGridClear}" != ""){
							/* var oldPowerGridInfoId = $("#oldPowerGridInfoId"+"${idx}").val();
							if(oldPowerGridInfoId != ids.join(",")){
								$("#oldPowerGridInfoId"+"${idx}").val(ids.join(",")); */
								var clears = [];
								clears = "${powerGridClear}".split(",");
