...
<head>
	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
			...
			//如果有父键值
			 if($("#parentIdId").val() != null && $("#parentIdId").val() != ''){
				//重新赋值
				 jp.post("${ctx}/sys/dict/getLabel","parentId="+$("#parentIdId").val(),
				 function(data){
					 if($("#parentIdName").val() == $("#label").val()){
						 $("#parentIdName").val(data);
					 }
				 });
			 }else{
				 $("#parentIdName").val("");
	...
	<form:form id="inputForm" modelAttribute="dictValue"  method="post" class="form-horizontal">
		...
		<table class="table table-bordered  table-condensed dataTables-example dataTable no-footer">
		   <tbody>
		      ...
		      <tr>
		          <td  class="width-15 active">	<label class="pull-right">父键值:</label></td>
		          <td class="width-35">
		          <sys:gridselect url="${ctx}/sys/dict/datas" id="parentId" name="parentId" value="${dictValue.parentId}" labelName="dictValue.label" labelValue="${dictValue.label}"
							 title="选择父键值" cssClass="form-control" fieldLabels="父标签|值键值|排序" fieldKeys="label|value|sort" searchLabels="父标签|值键值" searchKeys="label|value" ></sys:gridselect>
		          </td>
		      </tr>
		      <tr>
