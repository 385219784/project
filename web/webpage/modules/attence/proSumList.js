...
$(document).ready(function() {
	...
	/*var  head3={
			...
			var  head7={
					title : '<br/><center>总加班</center><center>(小时)</center><br/>',
					field : 'sumOverTime',
					align : 'right',
					valign : 'middle',
			        width:"55px",
    ...
    var  head8={
        title : '<br/><center>折算</center><center>总工日</center>',
        field : 'overTimeDay',
        align : 'right',
        valign : 'middle',
        width:"55px",
     ...
     headTop.push(head1,head2,proType/*,head3*/,head4,head5,head6,head7,head8);
	//动态获取列
     $.ajax({ 
    	         type: "post", 
    	 ...
    	         success: function(data){
	  					for(var  i=0;i<data.length;i++){
                            ...
                            var  four=
                                {
                                    title : '<center>折算</center><center>工日</center>',
                                    field : 'ovday'+zy.colNum,
                                    align : 'right',
                                    width:"40px"
                                }
					  			headTop.push(first);
					  			headButton.push(second,three,four);
	  		}	
	  	}
    	 });
	...
	$('#proSum').bootstrapTable({
		  ...
		  //请求方法
               ...
               url: "${ctx}/attence/workAttence/proSumData",
  ...
  function getIdSelections() {
	...
	function exportExcel(){
	 	$("#searchForm").attr("action","${ctx}/attence/workAttence/exportProSum");
	 	$("#searchForm").submit();
	...
	/**
	  ...
	  function view(id){//没有权限时，不显示确定按钮
		   ...
		   <shiro:hasPermission name="attence:workAttence:showView">
		  jp.openDialogView('查看项目工日汇总详情', "${ctx}/attence/workAttence/showView?id=" + id + "&startDate=" + $("#start").val() + "&endDate=" + $("#end").val(),'500px', '500px', $('#proSum'));
		  </shiro:hasPermission>
	  }
</script>