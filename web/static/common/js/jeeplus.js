...
(function() {
    ...
    //打开对话框(添加修改)
	   ...
	   //打开对话框(添加修改)
	   openDialogTreeTable: function(title,url,width,height, $table,$tree){
		...
		if(width.indexOf("%")>=0 || height.indexOf("%")>=0 ){
	   	...
	   	top.layer.open({
	   	    ...
	   	    yes: function(index, layero){
	   	         ...
	   	         iframeWin.contentWindow.doSubmit($table, index,$tree);
	   		  },
	   	   cancel: function(index){ 
	   	      }
	   	}); 
	   ...
	   //打开对话框(添加修改,自定义2个按钮)
	   definedOpenDialogBtnTwo: function(title,url,width,height, $table,btn1,btn2,operate1){
		   console.log("url:" + url);
		var auto = true;//是否使用响应式，使用百分比时，应设置为false
		if(width.indexOf("%")>=0 || height.indexOf("%")>=0 ){
			auto =false;
	   	...
	   	top.layer.open({
	   	    ...
	   	    yes: function(index, layero){
	   	         ...
	   	         iframeWin.contentWindow.doSubmit($table, index, operate1);
	   		  },
	   	   cancel: function(index){ 
	   	      }
	   	}); 
	 ...
	 //打开对话框(添加修改,自定义三个按钮)
	   definedOpenDialog: function(title,url,width,height, $table,btn1,btn2,btn3,operate1,operate2){
		   console.log("url:" + url);
		var auto = true;//是否使用响应式，使用百分比时，应设置为false
		if(width.indexOf("%")>=0 || height.indexOf("%")>=0 ){
			auto =false;
	   	...
	   	top.layer.open({
	   		...
	   		btn2: function(index, layero){
	   	         ...
	   	         iframeWin.contentWindow.doSubmit($table, index,  operate2);
	   	         return false
	   		  },
	   	   cancel: function(index){ 
	   	      }
	 ...
	 //打开对话框(查看)
	   customOpenDialogView :function(title,url,width,height, $table){
		   var auto = true;//是否使用响应式，使用百分比时，应设置为false
			if(width.indexOf("%")>=0 || height.indexOf("%")>=0 ){
				auto =false;
			}
	   ...
	   /**条件用户选择框,201710271548 byscc**/
	   openUserSelectDialogById:function(isMultiSelect,seachId, yesFuc){
		   top.layer.open({
			    type: 2, 
			    area: ['800px', '500px'],
			    title:"选择用户",
			    auto:true,
		        maxmin: true, //开启最大化最小化按钮
			    content: ctx+"/project/visit/userSelectById?isMultiSelect="+isMultiSelect+"&seachId="+seachId,
			    btn: ['确定', '关闭'],
			    yes: function(index, layero){
			    	var ids = layero.find("iframe")[0].contentWindow.getIdSelections();
			    	var names = layero.find("iframe")[0].contentWindow.getNameSelections();
			    	var phones= layero.find("iframe")[0].contentWindow.getPhoneSelections();
			    	var mobiles= layero.find("iframe")[0].contentWindow.getMobileSelections();
					if(ids.length ==0){
						jp.warning("请选择至少一个用户!");
						return;
					}
			    	// 执行保存
			    	yesFuc(ids.join(","),names.join(","),phones.join(","),mobiles.join(","));
