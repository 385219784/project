...
public class DoEveryDayNotify  extends Task{
	...
	public void run() {
		...
		//设置当前的毫秒钟为0
		  ...
		  if(!workDay.contains(dayForWeek(today))) {
			  searchDay.add(today);
		  }
		...
		if (userlist==null||userlist.size()==0) {
			...
			for(String  str:userlist) {
				...
				oa.setContent("您三日内有日程没有填写,请及时填写,如已填写,请忽略!");
				oa.setType("4");
				oa.setTitle("日程提醒");
				oa.setStatus("1");
				...
				if(leader!=null&&leader.size()>0) {
					for(String  str:leader) {
						if (StringUtils.isNotBlank(userName)) {
							OaNotify  oa=new  OaNotify();
							oa.setContent("以下人员在近三日内没有填写日程:"+userName+"如已填写,请忽略!");
							oa.setType("4");
							oa.setTitle("日程提醒");
							oa.setStatus("1");
							oa.setCreateBy(new User("1"));
