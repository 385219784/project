...
public class UserUtils {
	...
	public static final String CACHE_USERAREA_LIST = "userAreaList";
	/**
	 * 根据ID获取用户
	 * @param id
	 * @return 取不到返回null
	 ...
	 * @return 取不到返回null
	 */
	public static User getByWorkerNo(String loginName){
		User param = new User();
		param.setNo(loginName);
		User user = userMapper.getByWorkerNo(param);
		if (user == null){
			return null;
		}
		user.setRoleList(roleMapper.findList(new Role(user)));
...
//		getCacheMap().remove(key);
	...
	/**
	 * 获取当前用户授权的区域   20180313155030 @byscc
	 * @return
	 */
	public static List<UserArea> getUserAreaList(){
		@SuppressWarnings("unchecked")
		List<UserArea> areaList = (List<UserArea>)getCache(CACHE_USERAREA_LIST);
		User  user=getUser();
		if (areaList == null){
			//  如果是超级管理员,则取所有的区域
			 if (user.isAdmin()) {
				 areaList = userMapper.getAreaIds("1");
				 putCache(CACHE_USERAREA_LIST, areaList);
			}else {
				areaList = userMapper.getAreaIds(user.getId());
				putCache(CACHE_USERAREA_LIST, areaList);
			}
	 ...
	 * @return
	 */
	public static List<String> getUserAreaIdList(){
		List<UserArea> areaList = getUserAreaList();
		List<String> areaIdList= Lists.newArrayList();
		for(UserArea userArea:areaList) {
	 ...
	 * @return
	 */
	public static List<String> getBigAreaList(){
		
		List<String> areaIdList= areaMapper.getAreaListByType();
	 ...
	 * 获取车间列表
	 */
	public static List<Map<String, String>> getWorkAreaMap(){
		List<Map<String, String>> areaIdList= areaMapper.getWorkAreaMap();
