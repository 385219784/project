...
public class MailBoxService extends CrudService<MailBoxMapper, MailBox> {
	...
	//标记已读
	@Transactional(readOnly = false)
	public void markAlRead(String id) {
		mapper.markAlRead(id);
		
	}
	//标记重要
	@Transactional(readOnly = false)
	public void markImptant(String id) {
		mapper.markImptant(id);
		
	}
	//取消重要
	@Transactional(readOnly = false)
	public void cancleImptant(String id) {
		mapper.cancleImptant(id);
